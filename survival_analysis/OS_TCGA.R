...
#inputFile = '/home/seokju/Project/LiverCancer/TCGA_rev1/1.miRNA/3.ExpressionProfiling_noPVTT/miRDeep2_Family_7mer_Expression_neoplasm_Qnorm_RPMFiltered_Intersect.txt'
#outputD = '/home/seokju/Project/LiverCancer/TCGA_rev1/1.miRNA/3.ExpressionProfiling_noPVTT/SurvivalAnalysis_v2/'
inputFile = '/home/seokju/New/LiverCancer/TCGA/2.mRNA/3.expression/Bitseq_ResultTotal_3PseqUpdatedFa_GeneLevelExp_neoplasm_Qnorm_RPKMFiltered_Intersect.txt'
outputD = '/home/seokju/New/LiverCancer/TCGA/2.mRNA/3.expression/SurvivalAnalysis_v2/'

InfoTable <- read.table('/home/seokju/Project/LiverCancer/src_rev1/SurvivalAnalysis/renew2/TCGA_New.txt',header=T,row.names=3,sep='\t',check.names=F)
...
InfoTable.filtered <- InfoTable[,colnames(InfoTable) %in% c(covariates_ori,"OS","Survival")]
print(InfoTable.filtered$OS)
InfoTable.filtered <- InfoTable.filtered[InfoTable.filtered$OS >= 0 & ! is.na(InfoTable.filtered$OS) & InfoTable.filtered$OS <= 1825,]
...
print(InfoTable.uni.filtered$OS)
...
for (geneSymbol in rownames(expTable)){
  ...
  fitCPH.uni <- coxph(Surv(OS,Survival) ~ Exp, data=InfoTable.result.uni)
  ...
  for (i in 1:length(covariates)){
    ...
    temp.formular <- as.formula(paste("Surv(OS,Survival) ~ Exp + ",paste(covariates,collapse=" + "),sep=""))
  ...
  figureCPH <- survfit(Surv(OS,Survival) ~ Exp, data = InfoTable.result.uni,conf.type="log-log")
  p.value <- summary["ExpUpper",]$`Pr(>|z|)`
    
  write.table(summaryTable.uni,file=paste(outputD,"/OS_uni/summary/",geneSymbol,"_sig.txt",sep=""),row.names=T,col.names=T)
  write.table(validationTable.uni,file=paste(outputD,"/OS_uni/validation/",geneSymbol,"_sig.txt",sep=""),row.names=T,col.names=T)
  ...
  if (p.value <= 0.05){
    #ggsave(paste(outputD,"/OS/sig/",geneSymbol,".pdf",sep=""),plt$plot,dpi=300,width=6,height=6)
    write.table(summary,file=paste(outputD,"/OS//summary/",geneSymbol,"_sig.txt",sep=""),row.names=T,col.names=T)
    write.table(validationTable,file=paste(outputD,"/OS//validation/",geneSymbol,"_sig.txt",sep=""),row.names=T,col.names=T)
  } else {
    #ggsave(paste(outputD,"/OS//plot/",geneSymbol,".pdf",sep=""),plt$plot,dpi=300,width=6,height=6)
    write.table(summary,file=paste(outputD,"/OS//summary/",geneSymbol,".txt",sep=""),row.names=T,col.names=T)
    write.table(validationTable,file=paste(outputD,"/OS//validation/",geneSymbol,".txt",sep=""),row.names=T,col.names=T)
  }
}
warnings()
