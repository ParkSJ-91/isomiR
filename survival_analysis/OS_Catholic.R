...
inputFile = '/home/seokju/Project/LiverCancer/Catholic/1.miRNA/3.ExpressionProfiling_noPVTT/miRDeep2_Family_7mer_Expression_neoplasm_Qnorm_RPMFiltered_Intersect.txt'
outputD = '/home/seokju/Project/LiverCancer/Catholic/1.miRNA/3.ExpressionProfiling_noPVTT/SurvivalAnalysis_v2/'
...
InfoTable.filtered <- InfoTable.filtered[InfoTable.filtered$OS >= 0 & ! is.na(InfoTable.filtered$OS) & InfoTable.filtered$OS <= 60,]
InfoTable.uni.filtered <- InfoTable[,colnames(InfoTable) %in% c("OS","Survival")]
InfoTable.uni.filtered <- InfoTable.uni.filtered[InfoTable.uni.filtered$OS >= 0 & ! is.na(InfoTable.uni.filtered$OS) & InfoTable.uni.filtered$OS <= 60,]
...
for (geneSymbol in rownames(expTable)){
  ...
  if (sum(temp.exp.t.filtered.filtered.uni[,1]) == 0) {next}
  ...
  if (p.value <= 0.05){
    ...
    #ggsave(paste(outputD,"/OS/sig/",geneSymbol,".png",sep=""),plt$plot,dpi=300,width=6,height=6)
    ...
    #ggsave(paste(outputD,"/OS/plot/",geneSymbol,".png",sep=""),plt$plot,dpi=300,width=6,height=6)
