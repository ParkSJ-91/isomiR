...
def get_AlleleFrequency(rcDic, rcDic2, cutoff, dataset, isoformType):
    ...
    for pre in rcDic.keys():
        for arm in rcDic[pre].keys():
            ...
            if sum(map(lambda x: reads[x][1], range(len(reads)))) >= cutoff or sum(map(lambda x: reads[x][2], range(len(reads)))):
                ...
                temp_frequencies = map(lambda x: math.log(sum(map(lambda y: reads2[y][x] + 0.1, range(len(reads2)))) / float(reads2[5][x] + 0.1), 10), range(len(reads2[0])))
                if dataset == 'Catholic':
                ...
                afDic_for_compare_frequency_up[pre+'\t'+arm] = sum([reads[x][0]+0.1 for x in range(len(reads)) if x < 5]) / float(reads[5][0]+0.1)
                afDic_for_compare_frequency_down[pre+'\t'+arm] = sum([reads[x][0]+0.1 for x in range(len(reads)) if x > 5]) / float(reads[5][0]+0.1)
    ...
    for pre in rcDic2.keys():
        for arm in rcDic2[pre].keys():
            ...
            for i in xrange(len(totalsamples)):
                ...
                offsetList.append(sum([rcDic2[pre][arm][x][i] for x in range(len(rcDic2[pre][arm]))]))
        ...
        for trend in afDic[arm].keys():
            for pre in afDic[arm][trend]:
                ...
                for i in xrange(len(totalsamples)):
                    ...
                    isoform = sum([rcDic2[pre][arm][x][i] for x in range(len(rcDic2[pre][arm]))])
                    mature = rcDic2[pre][arm][5][i]
...
if __name__=='__main__':
    ...
    isoformType = 'all' # first or second (major isomiR)
