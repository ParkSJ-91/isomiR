...
def get_ReadCount(IsomiRD, totalsamples, armDic, infoDic):
    ...
    for i in xrange(len(totalsamples)):
        ...
        file = filter(lambda x: x.startswith(sample) and x.endswith('_OffsetPerfect.txt'), os.listdir(IsomiRD))
        if len(file) == 0:
            continue
        elif len(file) == 1: pass
        else:
            print file
            exit()
        f = open(IsomiRD + file[0]); lines = f.readlines(); f.close()
        tempDic = dict()
        for line in lines:
            line = line.strip().split('\t')
            mature = line[0]
        ...
        for arm in rcDic[pre].keys():
            ...
            for offset in range(len(rcDic[pre][arm])):
                ...
                if len(rcDic[pre][arm][offset]) == 0:
                    ...
                    temp += [np.median(normal+cancer), np.median(normal), np.median(cancer)]
...
def get_AlleleFrequency(rcDic, rcDic2, cutoff, dataset):
    ...
    for pre in rcDic.keys():
        for arm in rcDic[pre].keys():
            ...
            if reads[4][1] + reads[5][1] + reads[6][1] >= cutoff or reads[4][2] + reads[5][2] + reads[6][2] >= cutoff:
                ...
                afDic_for_compare_frequency[pre+'\t'+arm] = np.median(temp_frequencies)
                afDic_normal[pre+'\t'+arm] = np.median(temp_frequencies_N)
                afDic_cancer[pre+'\t'+arm] = np.median(temp_frequencies_C)
...
def intersect_Frequency(catAfDic_for_compare_frequency, tcgaAfDic_for_compare_frequency, tsinghwaAfDic_for_compare_frequency, cat_corr_A, tcga_corr_A, tsinghwa_corr_A):
    ...
    cat_FP = np.median(map(lambda x: catAfDic_for_compare_frequency[x], filter(lambda y: y.split('\t')[1] == 'FP' and y in cat_corr_A and y in tsinghwa_corr_A, catAfDic_for_compare_frequency.keys())))#; rv_cat_FP = fw_cat_FP
    cat_TP = np.median(map(lambda x: catAfDic_for_compare_frequency[x], filter(lambda y: y.split('\t')[1] == 'TP' and y in cat_corr_A and y in tsinghwa_corr_A, catAfDic_for_compare_frequency.keys())))#; rv_cat_TP = fw_cat_TP
    tcga_FP = np.median(map(lambda x: tcgaAfDic_for_compare_frequency[x], filter(lambda y: y.split('\t')[1] == 'FP' and y in tcga_corr_A, tcgaAfDic_for_compare_frequency.keys())))#; rv_tcga_FP = fw_tcga_FP
    tcga_TP = np.median(map(lambda x: tcgaAfDic_for_compare_frequency[x], filter(lambda y: y.split('\t')[1] == 'TP' and y in tcga_corr_A, tcgaAfDic_for_compare_frequency.keys())))#; rv_tcga_TP = fw_tcga_TP
    tsinghwa_FP = np.median(map(lambda x: tsinghwaAfDic_for_compare_frequency[x], filter(lambda y: y.split('\t')[1] == 'FP' and y in tsinghwa_corr_A and y in cat_corr_A, tsinghwaAfDic_for_compare_frequency.keys())))#; rv_tsinghwa_FP = fw_tsinghwa_FP
    tsinghwa_TP = np.median(map(lambda x: tsinghwaAfDic_for_compare_frequency[x], filter(lambda y: y.split('\t')[1] == 'TP' and y in tsinghwa_corr_A and y in cat_corr_A, tsinghwaAfDic_for_compare_frequency.keys())))#; rv_tsinghwa_TP = fw_tsinghwa_TP
...
def Calc_Correlation(rcDic, totalsamples, afDic):
    correlation = []
    for pre in rcDic.keys():
        for arm in rcDic[pre].keys():
            ...
            for i in xrange(len(totalsamples)):
                sample = totalsamples[i]
                m1 = 0
                if len(rcDic[pre][arm][4])==0:
                    m1 = 0
                else:
                    m1 = rcDic[pre][arm][4][i]
                p1 = 0
                if len(rcDic[pre][arm][6]) == 0:
                    p1 = 0
                else:
                    p1 = rcDic[pre][arm][6][i]
                mature = rcDic[pre][arm][5][i]
                
                offsetList.append(m1+p1)
                canoList.append(mature)
...
def write_rcDic(expDic, rcDic, medianRcDic, outputD, totalsamples, afDic, analysistype, afDic2, infoDic, armDic2, changedDic):
    ...
    for arm in afDic.keys():
        for trend in afDic[arm].keys():
            for pre in afDic[arm][trend]:
                ...
                # write proportion 
                tempSum = sum(map(lambda x: x[0], medianRcDic[pre][arm]))
                if arm == "FP":
                    writerProportion.write(pre + '\t' + '\t'.join(map(lambda x: str(x[0] / float(tempSum)), medianRcDic[pre][arm])) + '\n')
                ...
                expList = []; expList_mature = []
                rpmList = []; rpmList_M = []; rpmList_P = []; rpmList_MP = []
                ...
                for i in xrange(len(totalsamples)):
                    ...
                    expList.append(m1+p1)
                    expList_mature.append(mature)
                    rpmList.append(expDic[matureName][i])
                ...
                if arm == 'FP':
                    ...
                    writerFPExp.write(pre + '\t' + '\t'.join(map(str,expList)) + '\n')
                    writerMatureFP.write(pre + '\t' + '\t'.join(map(str,rpmList)) + '\n')
                    writerFPRPM.write(pre + '\t' + '\t'.join(map(str,rpmList_MP)) + '\n')
                    writerMatureRPM.write(pre + '_FP' + '\t' + '\t'.join(map(str,rpmList)) + '\n')
                    writerMatureExp.write(pre + '_FP' + '\t' + '\t'.join(map(str,expList_mature)) + '\n')
                    writerOffsetExp.write(pre + '_FP' + '\t' + '\t'.join(map(str,expList)) + '\n')
                    writerAll.write(pre + '_FP' + '\t' + '\t'.join(map(str,frequencyList)) + '\n')
                    if len(rpmList_MP) > 0:
                        writerOffsetRPM.write(pre + '_FP' + '\t' + '\t'.join(map(str,rpmList_MP)) + '\n')
                    ...
                    writerTPExp.write(pre + '\t' + '\t'.join(map(str,expList)) + '\n')
                    writerMatureTP.write(pre + '\t' + '\t'.join(map(str,rpmList)) + '\n')
                    writerTPRPM.write(pre + '\t' + '\t'.join(map(str,rpmList_MP)) + '\n')
                    writerMatureRPM.write(pre + '_TP' + '\t' + '\t'.join(map(str,rpmList)) + '\n')
                    writerMatureExp.write(pre + '_TP' + '\t' + '\t'.join(map(str,expList_mature)) + '\n')
                    writerOffsetExp.write(pre + '_TP' + '\t' + '\t'.join(map(str,expList)) + '\n')
                    writerAll.write(pre + '_TP' + '\t' + '\t'.join(map(str,frequencyList)) + '\n')
                    if len(rpmList_MP) > 0:
                        writerOffsetRPM.write(pre + '_TP' + '\t' + '\t'.join(map(str,rpmList_MP)) + '\n')
                    if len(rpmList_M) > 0 and sum(rpmList_M) > 0:
        ...
        for arm in dOffsetDic[pre].keys():
            ...
            for tempFpOffset in range(len(dOffsetDic[pre][arm])):
                ...
                for tpOffset in medianDic.keys():
                    medianSum += np.median(medianDic[tpOffset])
                    normalSum += np.median(normalDic[tpOffset])
                    cancerSum += np.median(cancerDic[tpOffset])
                ...
                for tpOffset in relativeMedianDic.keys():
                    median = np.median(relativeMedianDic[tpOffset])
                    ...
                    normalizedMedian = np.median(normalizedRelativeMedianDic[tpOffset])
            ...
            for fpOffset in relativeMedianDic_FP.keys():
                median = np.median(relativeMedianDic_FP[fpOffset])
...
if __name__=='__main__':
    ...
    cat_corr_N, cat_corr_C, cat_corr_A, cat_pDic= write_rcDic(catExpDic, rcDic_cat, medianRcDic_cat, catOutputD+ '/Frequency/', catTotalsamples, intersectFrequencyDic,'Frequency',catAfDic_for_compare_frequency, catInfoDic2, armDic2, changedDic)
    tcga_corr_N, tcga_corr_C, tcga_corr_A, tcga_pDic = write_rcDic(tcgaExpDic, rcDic_tcga, medianRcDic_tcga, tcgaOutputD+ '/Frequency/', tcgaTotalsamples, intersectFrequencyDic,'Frequency',tcgaAfDic_for_compare_frequency, tcgaInfoDic2, armDic2, changedDic)
    tsinghwa_corr_N, tsinghwa_corr_C, tsinghwa_corr_A, tsinghwa_pDic = write_rcDic(tsinghwaExpDic, rcDic_tsinghwa, medianRcDic_tsinghwa, tsinghwaOutputD + '/Frequency/', tsinghwaTotalsamples, intersectFrequencyDic,'Frequency',tsinghwaAfDic_for_compare_frequency, tsinghwaInfoDic2, armDic2, changedDic)
